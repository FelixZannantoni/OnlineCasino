...
pokerRouter.post("/addPlayer", async (req: Request, res: Response) => {
  ...
  const result = await pokerService.addPlayer(
    playerId,
    username,
    displayname,
    balance,
...
pokerRouter.put("/bet", async (req: Request, res: Response) => {
  const [playerId, gameId, betAmount]: [string, string, number] = [
    req.body.playerId,
    req.body.gameId,
    req.body.betAmount
  ];
...
pokerRouter.put("/raise", async (req: Request, res: Response) => {
  const [playerId, gameId, amount]: [string, string, number] = [
    req.body.playerId,
    req.body.gameId,
    req.body.amount
  ];
...
pokerRouter.get("/games", async (_req: Request, res: Response) => {
